...
trait ValidationErrorDisplayOps {
  ...
  implicit val syntaxErrorDisplay: DisplayOps[TransactionSyntaxError] = {
    ...
    case InvalidAccountLedgerAddress => "The account ledger compatible address in the transaction is invalid, 20 bytes are expected"
    case InvalidAccountLedgerAsset   => "Type 1 transactions must have exactly 1 output with a valid Account ledger compatible asset"
  }
